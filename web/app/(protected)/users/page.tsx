  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <h1 className="text-2xl font-bold text-[#374151]">Usuários</h1>
          ...
          <tbody>
            ...
            {items.map((u) => (
              <tr key={u.id} className="border-b border-gray-100">
                <td className="py-3 px-4 text-xs text-[#6B7280]">{u.id}</td>
                <td className="py-3 px-4 font-semibold text-[#374151]">
                  <Link href={`/users/${u.id}`} className="text-[#B55D05] hover:underline">
                    {u.name}
                  </Link>
                </td>
                <td className="py-3 px-4 text-[#6B7280]">{u.email}</td>
                <td className="py-3 px-4 text-[#6B7280]">{u.role ?? "-"}</td>
